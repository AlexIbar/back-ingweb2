...
controlador.crearInventario = (req, res) => {
    body =JSON.parse(req.body.informacion)
    console.log(req.file.filename)
    let data = {
        serial:body.serial,
        modelo:body.modelo,
        descripcion:body.descripcion,
        fotoEquipo:'.\\\\img\\\\uploads\\\\'+req.file.filename,
        precio: parseInt(body.precio),
        usuarioCargo:body.usuarioCargo,
        marca:body.marca,
        estadoEquipo:body.estadoEquipo,
        tipoEquipo:body.tipoEquipo
    }
    console.log(data.usuarioCargo)
    GetOne.getUser(data.usuarioCargo)
        .then(() => GetOne.getMarca(data.marca))
        .then(() => GetOne.getEstadoEquipo(data.estadoEquipo))
        .then(() => GetOne.getTipoEquipo(data.tipoEquipo))
        .then(() => GetOne.getSerialOrModel(data.serial, data.modelo))
        .then(() => {
            Schemas.inventarioConect.create(data, (err) => {
